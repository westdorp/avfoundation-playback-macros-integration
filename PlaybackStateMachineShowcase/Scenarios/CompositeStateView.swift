...
struct CompositeStateView: View {
    ...
    private func surfaceRow(_ surface: PlaybackSurface) -> some View {
        ...
        return HStack {
            ...
            if !isActive {
                Button("Activate") {
                    if let id = SurfaceID(surface.id) {
                        viewModel.activateSurface(id: id)
                    } else {
                        viewModel.recordInvalidSurfaceID(surface.id)
                    }
                }
                .controlSize(.mini)
                Button("Remove") {
                    if let id = SurfaceID(surface.id) {
                        viewModel.removeSurface(id: id)
                    } else {
                        viewModel.recordInvalidSurfaceID(surface.id)
                    }
                }
...
private final class ViewModel {
    ...
    func activateSurface(id: SurfaceID) {
        applyEvent(.activateSurface(id: id.rawValue))
    }

    func removeSurface(id: SurfaceID) {
        applyEvent(.unregisterSurface(id: id.rawValue))
    }

    func recordInvalidSurfaceID(_ rawID: String) {
        traceStore.append(category: .compositeDenied, label: "invalid surface id", detail: rawID)
    }
