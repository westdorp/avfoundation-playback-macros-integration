...
struct LiveStateMachineTests {
    ...
    func happyPath() {
        ...
        #expect(t2.isAllowed)
        #expect(machine.state == .live(streamID: "live-001", position: .atLiveEdge))
    }
    ...
    @Test("snapToEdge from live marks position atLiveEdge")
    func snapToEdgeFromLive() {
        let machine = LivePlaybackMachine(initialState: .live(streamID: "live-001", position: .behindLiveEdge))
    ...
    func stopAlwaysReturnsIdle() {
        let liveMachine = LivePlaybackMachine(initialState: .live(streamID: "live-001", position: .atLiveEdge))
